...
public class UnitStats
{
    ...
    public void TakeDamage(int damage, bool isBlocking)
    {
        ...
        // 여기에 "유효타는 최소 1"이라는 하한이 있었다. 경감이 겹쳐 0이 되면 단단한 유닛끼리
        // 서로 0을 때려 전투가 끝나지 않는다는 것이 근거였다.
        //
        // 걷어낸 이유는 그 근거가 방어 쪽에서 이미 무너져 있었기 때문이다. 막아낸 타격은 진작
        // 이 하한에서 빠져 있었는데(완전 방어는 피해 0이다), 그러고도 교착에 빠지지 않는다 —
        // 막은 타격도 강인도(poiseDamagePerHit)는 그대로 깎으므로 몇 번 막다 보면 가드가 뚫리고
        // 몇 초를 통째로 무너진 채(Stagger) 서 있게 된다. 완전히 흘려내도 "강인도를 쓰고
        // 시간을 번다"가 되는 것이지 무적이 아니다.
        //
        // 그 논리는 상시 경감과 감쇄 배율에도 똑같이 적용된다. 그래서 규칙을 하나로 맞췄다:
        // 계산 결과가 0이면 0이 들어간다. 완전 무효는 어디서 왔든 완전 무효다.
        //
        // 대신 강인도가 교착을 푸는 유일한 축이 되었으므로, 피해를 0으로 만드는 조합을 새로
        // 만들 때는 그쪽이 살아 있는지 함께 봐야 한다(UnitController.TakeDamage의 poiseDamage).
