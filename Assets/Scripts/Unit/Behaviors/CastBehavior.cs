...
// 마법사가 마력을 모으는 동작.
//
// SkillBehavior와 따로 둔 이유는 원작 설정 때문이다. 마법은 버튼 하나로 나가는 정해진 액티브가
// 아니라 연산하고 영창해 구현하는 현상이라, "스킬 하나를 재생한다"는 구조로는 담기지 않는다.
// 여기서는 무엇을 쓸지 먼저 정하고(SelectSpell), 그 마법이 요구하는 시간만큼 서서 마력을
// 모은 뒤에야 현상이 나간다(ExecuteSpell).
//
// 이 동작의 값어치는 "느리다"는 것 자체다. 서 있는 동안 받는 피해가 2.2배이고
...
//
// 붙잡히면 영창을 버린다는 판단은 여기 없다. 후퇴 가지가 이 가지보다 위에 있고 그 조건이
// 영창 중을 포함하므로(UnitBehaviorTree.WantsRetreat), 붙잡히는 순간 트리가 끊고 물러난다.
public class CastBehavior : UnitBehavior
{
    private float stateTimer;
    private bool startFailed;

    public CastBehavior(UnitController context) : base(context)
    {
    }
    ...
    protected override void OnEnter()
    {
        ...
        SpellSpec spell;
        Vector3 aimPoint;
        if (!unit.SelectSpell(out spell, out aimPoint))
        {
            // 조건을 볼 때는 맞았는데 그 사이 사라졌다(적이 죽었거나 흩어졌다).
            startFailed = true;
            return;
        }
        ...
        unit.BeginSpellCast(spell, aimPoint);
        stateTimer = unit.CurrentCastDuration;
    }
