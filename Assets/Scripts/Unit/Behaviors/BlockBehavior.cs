...
public class BlockBehavior : UnitBehavior
{
    ...
    public BlockBehavior(UnitController context) : base(context)
    {
    }
    ...
    // 자세를 잡은 뒤에는 그 자세가 스스로 풀릴 때까지 다른 판단을 하지 않는다.
    // 막는 도중에 물러나기로 마음을 바꾸면 방패를 내리는 그 순간을 그대로 맞는다.
    ...
    protected override BTStatus OnTick()
    {
        ...
        bool stillThreatened = unit.RefreshBlockThreat();
        unit.FaceBlockThreat();
        ...
        if (stillThreatened) stateTimer = unit.Stats.blockDuration;
        else stateTimer -= Time.deltaTime;

        if (stateTimer <= 0f) return BTStatus.Success;

        if (minHoldTimer > 0f) return BTStatus.Running;

        // 최소 시간을 채웠는데 나를 노리고 휘두르는 적이 더는 없다 — 계속 들고 있을 이유가 없다.
        // 오래 들고 있어 봐야 그동안 공격을 못 할 뿐이다.
        return stillThreatened ? BTStatus.Running : BTStatus.Success;
    }
