...
// 붙잡힌 원거리·리치 직군이 상대를 마주 본 채 한 번 물러나는 상태. "사거리를 되찾는다"가 전부다.
//
// 등을 보이고 달아나는 도주(FleeState)와 나눠 두었다. 같은 "물러나기"지만 두 동작이 공유하는
...
//
// 어느 쪽으로 갈지는 부르는 쪽이 고르지 않는다 — UnitController.RetreatState가 정한다.
public class EvadeState : UnitBattleState
{
    ...
    public override void Enter()
    {
        ...
        Vector3 away = context.GetSpacingRetreatDirection();

        // 도약 모션이 있으면 뛰고, 없는 리그(고블린)는 걸어서 뒤로 뺀다.
        bool started = context.DodgeAnimationDuration > 0.01f ? TryBeginLeap(away) : TryBeginBackpedal(away);

        // 어느 쪽도 뚫려 있지 않다 — 구석에 몰렸다. 벽에 붙어 굳어 있느니 돌아서서 싸운다.
        if (!started) ReturnToCombat();
    }
    ...
    public override void Update()
    {
        ...
        //
        // 여기서 context.HasMoveDestination을 보면 안 된다. 그건 MoveState/SearchState가 쓰는
        // 순찰 목적지 플래그라 이 상태는 세우지도 지우지도 않는다 — 순찰하다 들어오면 그 플래그가
        // 참인 채로 남아 후퇴가 영영 끝나지 않는다.
        if (backpedalTimer > 0f && !context.HasReachedDestination(destination)) return;
    ...
    private bool TryBeginLeap(Vector3 away)
    {
        ...
        Vector3 leapSpot;
        if (!context.TryFindRetreatSpot(away, context.Stats.evadeRange, out away, out leapSpot)) return false;
    ...
    private bool TryBeginBackpedal(Vector3 away)
    {
        ...
        Vector3 resolved;
        if (!context.TryFindRetreatSpot(away, context.Stats.evadeRange, out resolved, out destination)) return false;

        // 실제로 갈 수 있는 방향으로 유지 대상을 갱신한다. 그러지 않으면 유지 창 동안
        // 막힌 원래 방향이 계속 돌아와 같은 벽으로 다시 향한다.
        context.CommitRetreatDirection(resolved);

        // 간격을 되찾는 후퇴는 2.5m면 충분하다 — 짧게 끊는 편이 낫다.
        context.MoveTo(destination, context.BackpedalSpeed);
        PlayBackpedalAnimation();
        return true;
    }
    ...
    private void PlayBackpedalAnimation()
    {
        ...
        context.PlayRetreatAnimation(Mathf.Max(context.CurrentMoveSpeed, 0.05f));
    }
}
