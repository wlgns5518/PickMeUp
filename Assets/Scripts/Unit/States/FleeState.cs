...
// 등을 보이고 달아나는 상태. 안전해질 때까지 반복해서 달린다.
//
// 뒷걸음으로 사거리를 되찾는 회피(EvadeState)와 나눠 두었다 — 그쪽은 상대를 마주 본 채
...
//    거리가 안 나온다.
// 둘은 멈추는 조건만 다르다(StillPressured 참조). 달리는 방식은 같으므로 한 상태에 둔다.
public class FleeState : UnitBattleState
{
    ...
    public FleeState(UnitController context) : base(context)
    {
    }
    ...
    public override void Enter()
    {
        ...
        chasedOff = context.Stats.fleeByRunning && context.ShouldKeepDistance();
        ...
        Vector3 away = chasedOff ? context.GetSpacingRetreatDirection() : AwayFromTarget();
        ...
        // 거리를 도로 내줬다.
        Vector3 resolved;
        if (!context.TryFindRetreatSpot(away, context.SurvivalFleeDistance, out resolved, out destination))
        {
        ...
        if (chasedOff) context.CommitRetreatDirection(resolved);
    ...
    public override void Update()
    {
        ...
        // 참인 채로 남아 도주가 영영 끝나지 않는다.
        if (fleeTimer > 0f && !context.HasReachedDestination(destination)) return;
    ...
    {
        return chasedOff ? context.IsBeingChased() : context.ShouldRetreatForSurvival();
    }
    ...
    private Vector3 AwayFromTarget()
    {
        ...
        away.y = 0f;
        if (away.sqrMagnitude <= 0.0001f) return -context.transform.forward;
